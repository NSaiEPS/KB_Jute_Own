...
const ProductPartyWise = ({navigation}) => {
useLayoutEffect(()=>{
...
const [productModelOpen, setproductModelOpne] = useState(false)
const [finalSearchproduct, setfinalSearchproduct] = useState('')

  return (
    <SafeAreaView>
    <View style={styles.ProductPartyWiseTop}>
      <View style={styles.ProductPartyWiseTopBtn}>
      <Button 
      title='Create PDF'
      containerStyle={{
        width:130,
      ...
      <View style={styles.ProductPartyWiseTopTexts}>
        <View>
...
<View style={[{...commonStyles.createOrderView,marginTop:15, width:'80%'}]}>
      ...
      <TouchableOpacity onPress={()=>setproductModelOpne(true)}>
      <View style={[{...commonStyles.textInput,height:50,justifyContent:'center', borderWidth:1, borderRadius:0}]}>
     <Text style={{fontSize:15,fontWeight:'400',color:colors.black, marginLeft:2 }}>
      {'Select Product'}
     </Text>
 ...
 <SearchModels 
      searchType={'Select Product'}
      modelopen={productModelOpen}
      setModelOpen={setproductModelOpne}
      animationType={'slide'}
      finalSearch={finalSearchproduct}
      setfinalSearch={setfinalSearchproduct}
 ...
 <SearchModels 
   ...
   <Picker
     selectedValue={stockInfo.selectLocation}
...
export default ProductPartyWise

const styles = StyleSheet.create({
  ProductPartyWiseTopBtn:{
    flexDirection:'row',
    width:'80%',
    marginLeft:'auto',
    marginRight:'auto',
